...
registry-list-local: ## List servers from locally cloned registry
	@if [ ! -d "mcp-registry" ]; then echo "❌ Local registry not found. Run 'make registry-clone-direct' first."; exit 1; fi
	@echo "📦 Локальные MCP серверы:"
	@find mcp-registry/servers -maxdepth 1 -type d -not -path "mcp-registry/servers" | sort | while read dir; do \
		...
		if [ -f "$$dir/server.yaml" ]; then \
			echo "  ✅ $$server (server.yaml найден)"; \
		else \
			echo "  ⚠️  $$server (server.yaml не найден)"; \
		fi; \
	done
